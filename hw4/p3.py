...
class Generator(nn.Module):
    def __init__(self, inputSize=1600, hiddenSize=400, outputSize=1600):
        super(Generator, self).__init__()
        self.main = nn.Sequential(
            nn.Linear(inputSize, hiddenSize*2),
            nn.BatchNorm1d(hiddenSize*2),
            nn.ReLU(True),

            nn.Linear(hiddenSize*2, hiddenSize*2),
            nn.BatchNorm1d(hiddenSize*2),
            nn.ReLU(True),
...
def predict(model, G, data_loader):
    prediction_results = []
    with torch.no_grad():
        ...
        for i, (data, target) in enumerate(data_loader):
            ...
            M = 100
            fixed_noise = torch.randn(M * N_way, 1600, device=device)
            #proto : 15*1600
            for num in range(N_way):
                for m in range(M):
                    if num == 0 and m == 0:
                        sample = fixed_noise[num+m] + proto[num]
                    else:
                        sample = torch.cat([sample, fixed_noise[num+m] + proto[num]], dim=0) 
            sample = sample.view(M * N_way, -1)
            #sample : (M*way)*1600
            sample = G(sample)           
            proto = proto.reshape(N_shot, N_way, -1)
            sample = sample.reshape(M, N_way, -1)
            proto = torch.cat([proto, sample], dim=0)          
            proto = proto.reshape(N_shot + M, N_way, -1).mean(dim=0)
...
if __name__=='__main__':
    ...
    if use_cuda == True:
        model = torch.load('p3_model/M')
        G = torch.load('p3_model/G')
    else:
        model = torch.load('p3_model/M', map_location=torch.device('cpu'))
        G = torch.load('p3_model/G', map_location=torch.device('cpu'))
    prediction_results = predict(model, G, test_loader)
